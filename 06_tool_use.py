"""
Lesson 6: Tool use with manual function calling.

This script shows the lowest-level tool-calling pattern: define tool schemas,
map them to Python functions, and loop until the model returns a final answer.
"""
...
def tool_calling_agent(user_message: str) -> str:
    ...
    # Keep the loop short so a bad tool plan cannot run forever.
    for iteration in range(5):  # Max 5 iterations to avoid infinite loops
        ...
        for tool_call in message.tool_calls:
            # Parse the tool arguments that the model requested.
